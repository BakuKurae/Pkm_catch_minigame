//Se ha implementado un sistema de deteccion de errores respecto al archivo que almacena el registro de los logros //<>//
//se arreglo el problema del boton para generar pkm shiny (solo se puede presionar una vez antes de hacer reroll o capturar el pkm shiny generado)
...
boolean shinyActive = false;//false -> boton de pkm shiny sin presionar, true -> el siguiente es un pkm shiny asegurado
...
void setup() {
  ...
  if (fileExists(dataPath("cache.txt"))) {
    Arch = loadStrings("cache.txt");//archivements
    if (Arch.length != 4) {
      println("Ha habido un error al leer el archivo de logros.");
      output = createWriter("data/cache.txt");
      for (int j = 0; j <= 3; j++) {
        output.println("0");
      }
...
void draw() {
  ...
  if (stock_field == 15) {
    ...
    if (Integer.parseInt(Arch[0]) == 0) {
      ...
      text("Maestro recolector de Shinys", 512, 530);//Todos los pkm capturados son shiny
    }
    ...
    if (Integer.parseInt(Arch[2]) == 0) {
      ...
      text("Comunidad de legendarios", 512, 570);//Ganar habiendo capturado un mewtwo, mew, zapdos, articuno y moltres
    }
    ...
    if (textCheck == 0) {//condicion para permitir al texto imprimirse una vez
      ...
      text("El costo por re-roll es: $2\n El costo por pokeball: $5\n Si tienes poco dinero...\n presiona los pokemon en el almacen para venderlos.\n Pierdes si agotas el dinero y pokeballs.\n Ganas al conseguir atrapar 15 pokemon.", 770, 185);
      text("Asegura la captura en un 100% de exito ($10)", 733, 308);
    ...
    if (circleOver && countPKB != 360 && stock_field != 15) { //si el mouse se encuentra sobre el boton y aun quedan pokeballs para lanzar y existen espacios vacion en el almacen [miniaturas]
      if (switchPokeball) {
        ...
        if (round(random(9))+1 <= gotcha) {//calculo de captura pkmn 
          ...
          if (cont_captura != 5) {//reset de cantidad de captura
            ...
            cont_captura=0;
            money = money+40; //incremento bonus por capturar 6 pkm
          }
      ...
      if (stock[floor((mouseX-11)/32)] == 1) {
        if (shinyStock[floor((mouseX-11)/32)] == 1) {//Si esta condision se cumple, significa que en esta posicion se almacena un pkm shiny y al ser eliminado, regresara mas dinero
          money = money+40;//incremento de dinero por pkm atrapado (40)
          println("Cantidad actual: " + "$ " + money);
        } else {
          money = money+20;//incremento de dinero por pkm atrapado (20)
          println("Cantidad actual: " + "$ " + money);
        }
      } else {
      ...
      switchPokeball = true;
    } else if (percentOver && money >= 20 && shinyActive == false) {
      money = money - 20;//costo por forzar la aparicion de un pkm shiny
      shinyActive = true;
      percent();
    }
    scenario();
